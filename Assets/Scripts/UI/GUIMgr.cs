...
public class GUIMgr : MonoBehaviour {
    ...
    void Awake()
    {
        whiteToMoveTr = transform.FindChild("WhiteTurnText");
        blackToMoveTr = transform.FindChild("BlackTurnText");

        whiteToMoveTr.gameObject.SetActive(false);
        blackToMoveTr.gameObject.SetActive(false);

        whiteScoreText = transform.FindChild("WhiteScoreText").GetComponent<Text>();
        blackScoreText = transform.FindChild("BlackScoreText").GetComponent<Text>();
